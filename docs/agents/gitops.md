...
- `.github/workflows/cargo-audit.yml` runs a weekly (plus manual `workflow_dispatch`) non-blocking `cargo audit` against `Cargo.lock`. The project-local `.cargo/audit.toml` carries four accepted advisories: ADR-0069's three stale-rustls advisories plus the unreachable `rkyv` advisory accepted in #145. Findings beyond the list open a `needs-triage` tracking issue deduplicated by exact title while the run stays green, so an external advisory publication never reddens `main` or touches the merge gate.
