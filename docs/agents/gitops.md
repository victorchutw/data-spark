...
4. Move every `Unreleased` entry in `CHANGELOG.md` under a new
   `## [<version>] - <YYYY-MM-DD>` heading dated with the release date,
   leaving `Unreleased` empty, and add the version's GitHub Release link
   beside the other link definitions at the bottom of the file. The release PR
