...
- A PR opened ready for review is reviewed immediately, in parallel with the
  ...
  for review.
- A review takes about 2-3 minutes. The `rust` CI job takes about 3-5
  minutes with a warm dependency cache (2m39s measured on an empty
  change) and over an hour on a cold cache (toolchain bump, `duckdb`
  upgrade, or cache eviction), so review comments and warm-cache CI
  green land in the same few-minute window.
