...
are not illustrations — `cargo test --locked` runs every one of them against the
real binary and checks its load report, so an example that stops working fails
CI ([tests/examples.rs](../tests/examples.rs)).
...
| [jsonl-to-parquet-full-refresh](jsonl-to-parquet-full-refresh/) | JSONL into a Parquet dataset directory; a second run replaces rather than accumulates. |
...
Every example uses paths relative to its own directory, so run the binary from
inside one:

```bash
cd examples/csv-to-duckdb-full-refresh
data-spark load customers-load.yml
```

A load writes into the directory it runs in: the destination dataset, the
artifact directory (`.data-spark/runs/<load-id>/` by default), and — for the
pinned-schema examples — the pinned schema file the first load bootstraps. To
keep the repository tree clean, copy an example somewhere writable first, which
is what the test does:

```bash
cp -r examples/csv-to-duckdb-full-refresh /tmp/ && cd /tmp/csv-to-duckdb-full-refresh
data-spark load customers-load.yml
```
